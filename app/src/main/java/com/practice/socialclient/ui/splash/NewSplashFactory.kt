...
class NewSplashFactory() : ViewModelProvider.Factory {
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        val viewModel: ViewModel
        viewModel = if (modelClass == SplashViewModel::class.java) {

//            DaggerNewFriendsComponent.builder()
//                .newAppModule(App.instance?.getAppModule())
//                .build()
//                .createFriendsModel()
DaggerNewSplashComponent.builder().newAppModule(App.appModule).build().createSplashModel()
        } else {
